...
namespace cvPipeline{
    namespace Filter{
        ...
        ImageProcessor::ImageProcessor(): enable(true)
        {
            inputMat[""] = 0;
        ...
        void ImageProcessor::defInputMat(const std::string& name)
        {
            ...
            inputMat[name] = 0;
            inputMatDescription[name] = std::string("");
        }
        ...
        void ImageProcessor::undefInputMat(const std::string& name)
        {
            ...
            for(; input!=inputMat.end(); input++){
                if(desc->first == name){
                    inputMatDescription.erase(name);
                    break;
                }
            }
        }
        ...
        ImageProcessor& ImageProcessor::operator >> (ImageProcessor& dst)
        {
        ...
        ImageProcessor& ImageProcessor::operator >>(ImageProcessor::FilterInput& mnp)
        {
        ...
        ImageProcessor& ImageProcessor::setInputMat(const std::string& name, const cv::Mat& mat)
        {
            checkInputMatExists(name);
            inputMat[name] = &mat;
                ...
                if(entry.name == name){
                    if(entry.creator != 0){
                        ...
                        if(filter == 0){
                            ...
                            std::cerr << "ImageProcessor::createFilter(" << name << ")" << typeid(*filter).name() << " created." << std::endl;
                        }
#endif
                        return filter;
                    }else{
#ifdef _DEBUG
                        std::cerr << "ImageProcessor::createFilter(" << name << ") fail to create. Null factory." << std::endl;
#endif
                        return 0;
                    }
                }
...
#ifdef _DEBUG
                std::cerr << "[" << i << "] ImageProcessor::entryFilter(" << name << ") factory entried" << std::endl;
#endif
                    return;
                }
            }
        ...
        void ImageProcessor::putMarkdown(std::ostream& stream) const
        {
            ...
            for(; mapEntry!= inputMat.end(); mapEntry++){
                ...
                std::string inputDescription = " - input image";
                if(inputMatDescription.count(name) > 0){
                    inputDescription = inputMatDescription.at(name);
                    if(inputDescription != ""){
                        inputDescription = std::string(" - ") + inputDescription;
                    }
                }
                if(name == ""){
